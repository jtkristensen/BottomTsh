...
leqMono :: n `Leq` m -> SNat k -> n `Leq` (m :+ k)
leqMono (ZLeqN     m) k = ZLeqN (m %+ k)
leqMono (SLeqS nLeqm) k = SLeqS (leqMono nLeqm k)

leqMonoPlus :: n `Leq` m -> SNat k -> (n :+ k) `Leq` (m :+ k)
leqMonoPlus (ZLeqN     m) k = plusComm m k %| leqMono (leqReflexive k) m
